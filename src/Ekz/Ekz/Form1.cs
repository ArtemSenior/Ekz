...
namespace Ekz
{
    public partial class Form1 : Form
    {
        ...
        void f1()
        {
            ...
            {
                MessageBox.Show("Некорректный ввод данных. Повторите попытку.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            double a, b, c;
            double x1 = 1.0, x2 = 3.0, x3 = 1.0;
            double y1 = 2.0, y2 = 0.0, y3 = -2.0;
            a = (x1 - x) * (y2 - y1) - (x2 - x1) * (y1 - y);
            b = (x2 - x) * (y3 - y2) - (x3 - x2) * (y2 - y);
            c = (x3 - x) * (y1 - y3) - (x1 - x3) * (y3 - y);
            
            if ((x == 1 && y == 1) || (x == 1 && y == -1) || (x == 1 && y == 0))
            {
                MessageBox.Show("Точка находится внутри");
                toolStripStatusLabel1.Text = "Точка находится внутри";
            }
            else
                if (a == 0.0 || b == 0.0 || c == 0.0)
            {
                MessageBox.Show("Точка находится на границе");
                toolStripStatusLabel1.Text = "Точка находится на границе";
            }
            else if(((x - 1) * (x - 1) + y * y <= 4) && (y <= 3 - x) && (y >= x - 3))
            {
                MessageBox.Show("Точка находится внутри");
                toolStripStatusLabel1.Text = "Точка находится внутри";
            }
            ...
            {
                MessageBox.Show("Точка находится вне");
                toolStripStatusLabel1.Text = "Точка находится вне";
            }
        }
